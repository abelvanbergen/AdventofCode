class Monkey:
	...
	def evaluate(self, item, monkeys, magic_number):
		self.items_evaluated += 1
		if self.operator == "*":
			worry_level = (item * self.get_operator_value(item))
		else:
			worry_level = (item + self.get_operator_value(item))
		if (worry_level % self.testvalue == 0):
			monkeys[self.true].items.append(worry_level % magic_number)
		else:
			monkeys[self.false].items.append(worry_level % magic_number)
...
for i, m in enumerate(monkey_data):
	...
	magic_number *= monkeys[i].testvalue
for i in range(10_000):
	for monkey in monkeys:
		for item in monkey.items:
			monkey.evaluate(item, monkeys, magic_number)
		monkey.items.clear()
item_evaluations = sorted([m.items_evaluated for m in monkeys])
print(item_evaluations)
